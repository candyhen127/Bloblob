...
{
    /*
    [SerializeField] float requiredTotalMass = 10f; // Mass needed to activate
    ...
    // Use a list to keep track of items to correctly manage mass on exit
    private List<Rigidbody2D> itemsOnPlate = new List<Rigidbody2D>();
    ...
    void FixedUpdate()
    {
        if (currentTotalMass >= requiredTotalMass && !isActivated)
        {
            isActivated = true;
            Debug.Log("Pressure Plate ON! Total Mass: " + currentTotalMass);
            onPlateActivated.Invoke(); // Trigger Unity events for activation
        }
        else if (currentTotalMass < requiredTotalMass && isActivated)
        {
            isActivated = false;
            Debug.Log("Pressure Plate OFF! Total Mass: " + currentTotalMass);
    ...
    {
        if (other.CompareTag("Player") || other.CompareTag("Blob"))
        {
            Rigidbody2D rb = other.GetComponent<Rigidbody2D>();
            if (rb != null && !itemsOnPlate.Contains(rb))
            {
                itemsOnPlate.Add(rb);
                UpdateTotalMass();
            }
    ...
    {
        currentTotalMass = 0f;
        foreach (Rigidbody2D itemRb in itemsOnPlate)
        {
            // Only add mass if the item still exists and is not null
            if (itemRb != null)
            {
                currentTotalMass += itemRb.mass;
            }
        }
    }
    */
